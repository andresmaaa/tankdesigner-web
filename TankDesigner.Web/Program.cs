...
// Permite usar estado de autenticaci�n en Blazor
builder.Services.AddCascadingAuthenticationState();

// Permite acceder al HttpContext desde servicios
builder.Services.AddHttpContextAccessor();

// Configuraci�n para Railway / proxy inverso
...
// Obtiene la cadena de conexi�n desde configuraci�n
var connectionString = builder.Configuration.GetConnectionString("DefaultConnection")
    ?? throw new InvalidOperationException("No se ha encontrado la cadena de conexi�n 'DefaultConnection'.");
...
// Configuraci�n de Identity (usuarios, roles, login, etc.)
builder.Services
    .AddIdentity<ApplicationUser, IdentityRole>(options =>
    {
        // Pol�tica de contrase�a segura y profesional
        ...
        // Sin confirmaci�n de email obligatoria de momento
...
// Configuraci�n de DataProtection
...
builder.Services.ConfigureApplicationCookie(options =>
{
    ...
    // Renovaci�n autom�tica mientras navega
...
// Registro de servicios propios de la aplicaci�n
...
// Configuraci�n de errores en producci�n
    ...
    InvitacionesService invitacionesService) =>
{
    ...
    if (string.IsNullOrWhiteSpace(nombreCompleto))
        return Results.Redirect(BuildRegisterRedirect("Debes indicar un nombre completo v�lido."));

    if (string.IsNullOrWhiteSpace(email))
        return Results.Redirect(BuildRegisterRedirect("Debes indicar un correo electr�nico v�lido."));

    if (!new EmailAddressAttribute().IsValid(email))
        return Results.Redirect(BuildRegisterRedirect("El correo electr�nico no es v�lido."));

    if (string.IsNullOrWhiteSpace(password))
        return Results.Redirect(BuildRegisterRedirect("Debes indicar una contrase�a v�lida."));

    if (password.Length < 8)
        return Results.Redirect(BuildRegisterRedirect("La contrase�a debe tener al menos 8 caracteres."));

    if (!string.Equals(password, confirmPassword, StringComparison.Ordinal))
        return Results.Redirect(BuildRegisterRedirect("Las contrase�as no coinciden."));
    ...
    if (usuarioExistente != null)
    {
        ...
        if (tienePassword)
            return Results.Redirect(BuildRegisterRedirect("Ya existe una cuenta registrada con ese correo electr�nico."));
    ...
    if (!resultadoRol.Succeeded)
        return Results.Redirect(BuildRegisterRedirect("La cuenta se cre�, pero no se pudo asignar el rol Usuario."));
...
// Endpoint POST para solicitar recuperación de contraseña
    ...
    EmailService emailService) =>
{
    ...
    // No se informa si el correo existe o no para evitar enumeración de usuarios.
    ...
    var cuerpoHtml = $"""
        <div style="font-family: Arial, sans-serif; color: #1f2937; line-height: 1.5;">
            ...
            <p>Hemos recibido una solicitud para cambiar la contraseña de tu cuenta de Tank Structural Designer.</p>
            <p>Pulsa en el siguiente botón para crear una nueva contraseña:</p>
    ...
    try
    {
        await emailService.EnviarEmailAsync(email, "Recuperar contraseña - Tank Structural Designer", cuerpoHtml);
    }
    catch
    {
...
// Endpoint POST para aplicar nueva contraseña
    ...
    UserManager<ApplicationUser> userManager) =>
{
    ...
    if (string.IsNullOrWhiteSpace(password))
        return Results.Redirect(BuildResetRedirect("Debes indicar una contraseña válida."));

    if (password.Length < 8)
        return Results.Redirect(BuildResetRedirect("La contraseña debe tener al menos 8 caracteres."));

    if (!password.Any(char.IsUpper))
        return Results.Redirect(BuildResetRedirect("La contraseña debe incluir al menos una mayúscula."));

    if (!password.Any(char.IsLower))
        return Results.Redirect(BuildResetRedirect("La contraseña debe incluir al menos una minúscula."));

    if (!password.Any(char.IsDigit))
        return Results.Redirect(BuildResetRedirect("La contraseña debe incluir al menos un número."));

    if (!password.Any(c => !char.IsLetterOrDigit(c)))
        return Results.Redirect(BuildResetRedirect("La contraseña debe incluir al menos un símbolo."));

    if (!string.Equals(password, confirmPassword, StringComparison.Ordinal))
        return Results.Redirect(BuildResetRedirect("Las contraseñas no coinciden."));
    ...
    {
        return Results.Redirect(BuildResetRedirect("El enlace de recuperación no es válido."));
    }

    var resultado = await userManager.ResetPasswordAsync(usuario, token, password);

    if (!resultado.Succeeded)
        return Results.Redirect(BuildResetRedirect("El enlace ha caducado o la contraseña no cumple los requisitos. Solicita un nuevo enlace."));
